...
const message = (id: string, seq: number, role: DraftMessage["role"], text: string): DraftMessage => ({
  ...
  seq,
  role,
  parts: [{ type: "text", text, source: "fixture", version: 1 }],
});
  ...
  sessions: [
    {
      ...
      pending: 0,
      messages: [
        message("shell-1", 1, "user", "先搭好壳，让手机也能轻松浏览。"),
        message("shell-2", 2, "assistant", "正在整理页面结构。这是静态样本，不代表真实执行进度。"),
      ],
    },
    {
      id: "review",
      ...
      pending: 1,
      messages: [
        message("review-1", 1, "assistant", "待答演示：需要你确认交互方向。此切片只读，不提供批准或提交操作。"),
      ],
    },
    {
      id: "notes",
