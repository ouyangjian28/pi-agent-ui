...
export function transition(from, to) {
    if (from === to)
        return null; // 幂等重放
    if (from === "done")
        return `monotonic: done is terminal, reject ${to}`; // 单调性：done 后一切降级拒
    if (from === "expired")
        return to === "done" ? null : `expired only closes to done (late completion), reject ${to}`;
    // received→derived→started→done 主线；expired=任意未终态可入
    const order = ["received", "derived", "started"];
    const fromIdx = order.indexOf(from);
    const toIdx = order.indexOf(to);
    if (to === "expired")
        return null;
    if (to === "done")
        return from === "started" || from === "derived" ? null : `received cannot jump to done (需经过派生/开栓), reject done from ${from}`;
...
export function lateReplayAfterDone(last, event) {
    if (event === "redeliver") {
        if (last === "done")
