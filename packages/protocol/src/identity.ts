...
export interface EntryIdentity {
  ...
  readonly lengthHash: string; // 内容长度哈希（截短/换代检测）
}
...
export function attachmentIdentity(multiset: AttachmentMultiset): string {
...
export function matchKeyOf(text: string, attachments: AttachmentMultiset, ordinal: number): IntentMatchKey {
...
export interface WriteAuthority {
  readonly sessionId: SessionId;
  readonly revision: number; // 单调递增；提交须持最新
  readonly writerEpoch: number; // 写权代次：接管/回收递增；旧 epoch 提交拒
  readonly processGeneration: number; // 进程代次
}
