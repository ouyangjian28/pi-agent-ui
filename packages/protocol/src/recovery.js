...
import { fileTailSatisfiesClauseZero, intervalClosedByFinalAnswer, intervalToolCallsPaired } from "./session-file.ts";
function buildGroups(entries, intents) {
    const groups = new Map();
    for (const e of entries) {
        if (e.corrupt)
            continue; // 坏行不参与匹配也不污染归属
    ...
    for (const j of intents) {
        ...
        if (!out.has(k))
            out.set(k, { key: j.matchKey, groupEntries: groups.get(k) ?? [] });
    }
    return out;
}
...
function groupAmbiguous(group, members) {
...
function occupiedEntryIds(intents, extraAnchors) {
    const occ = new Set();
    for (const j of intents) {
...
export function runRecovery(input) {
    ...
    const verdicts = [];
    const newConsumed = [];
    const extraAnchors = new Map(); // 第一遍新增锚（第二遍③用）
    const pendingFinalCheck = []; // 第二遍待检集合（十八审②：命中/自有证据同入，第一遍不终检）
    ...
    for (const j of intents) {
        if (j.cancelled)
            verdicts.push({ intentId: j.intentId, state: "cancelled", reason: "clear 行重放：身份确定+执行未终局→cancelled" });
    }
    // 第一遍（身份与消费，逐意图串行）
    for (const j of intents) {
        if (j.cancelled)
    ...
    for (const j of intents) {
        ...
        if (v.state !== "delivered")
            break; // 规格口径：水位推进=终态（delivered/settled/unknown 终裁）才推进；inflight/未判=停
