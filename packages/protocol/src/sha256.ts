// 3b2b-R4/R5（GPT 3b-2b 审读）：协议包自持 SHA-256。
// 需求来源：①附件身份 canon=「每图 SHA-256 前 12hex 多重集」（identity.ts 三审定界冻结，
//  session 读侧派生必须同构——此前 fnv1a64(type,id,url) 与写侧永不相交且真实图片块
//  {type:'image',data,mimeType} 无 id/url 会全部塌缩同一 id）；②§1.3 源文件指纹=整文件
//  SHA-256（journalFingerprint/sessionFingerprint 双指纹接线）。
// 纯 TS 实现（32 位字运算+TextEncoder 取 UTF-8 字节）：不引 node:crypto——protocol 包
// 保持运行时无关（服务器/浏览器同构可用）；正确性由 golden 向量测试钉死（FIPS 180-4 参考值）。
...
export function sha256Hex12(input: string): string {
