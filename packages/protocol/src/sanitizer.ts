...
//   - POSIX 多段路径（3b2e-C4）：{2,32} 段+尾随余段整体消费（[path] 单次替换——深路径 >32 段
//     一并吃尽，替换缩短不回拉界外尾）；首段 >255+≥1 后继段整遮；超长单段仍不遮（声明语义）；Windows 路径 >255 尾随吃满令牌（N9 修复）
...
const SECRET_REPLACEMENTS: readonly { readonly re: RegExp; readonly out: string }[] = [
  ...
  //    C4（3b2e/GPT 3b2d D12）：尾随 [^\s"'<>|\\]* 吃满令牌余段——已识别多段路径整体消费
  //    （超长末段/深路径 >32 段不再只遮前缀把尾拉回预览；类含 / 故余下各段一并吃尽）。
  { re: /(?<!:)(?:\/[^\s"'<>|\\]{1,255}){2,32}[^\s"'<>|\\]*|(?<!:)\/[^\s"'<>|\\]{256,}\/[^\s"'<>|\\][^\s"'<>|\\]*/g, out: "[path]" },
