...
| **C5-06 归因前向区间**（session-attribution.ts 纯投影：区间=[consumed.anchorEntryId 身份, intervalEnd{entryId,lengthHash}] 从锚向后含两端——assistant-A 归 A 非 B（方向修正）；终点哈希不符→null 不猜；嵌套区间内层优先；多次 consumed 最新生效；**扫描置换不敏感**（两源扫描先后无关=纯集合关系）；宿主 entryId 唯一化（blockIndex 后缀）入投影前） | session-attribution.test 9 it（①GPT 定向反例③哈希伪终点④不可见/倒置⑤嵌套⑥最新⑦置换⑧system/toolResult⑨块键约束）；M-a06（方向反转外层优先）→1 挂/M-a07（哈希不验）→1 挂 | ✅ |
| **C5-07 纯投影装页+单模型**（projection-frames.ts：packPage 双上限+首条必装+单条超限→null 宿主转错误帧+offset 域外→null（4409）；buildRecoveryFrame=单 offset 驱动 perIntent+unknown/resumable 恒整表；buildSessionsFrame=页级 listReliability 聚合+字节装页；sessions 帧加页级 listReliability 字段；文档九处单模型对齐（§3.5 前向区间/§4 快照口径/perIntent provisional 表/evidenceHash canonical 数组/32MB→理论估算/timeSource 删/previewTruncated→textPreview.truncated）） | projection-frames.test 5 it（装页双上限/域外/尾页/recovery 整表/页级聚合/单条超限）；M-p07（首条不失败）→1 挂 | ✅ |
