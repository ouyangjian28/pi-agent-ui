...
| **变异八组（基线 fc4ab4f）** | M-B2a genNotice 身份门失效（取槽现态当本代）→**N4b 挂**（carry=2 同槽换代：旧闭包落在活槽上，身份门是唯一防线；N4 原 stopA 后 slot-reaped=死槽双重保护，杀不到）；M-B2b genError 身份门失效→N3 审计断言挂；M-B3 settleLoadRef 去恰一次守卫→2 挂；M-B4 watchFile 去 rec 复核→1 挂；M-B5 投影回退盲取→5 挂；M-B6a ⑧规则去尾随 \S*→3 挂；M-B6b maskPem 无 END 不收口（break 跳内层）→重复 BEGIN 向量挂；**M-B1 结构性存活披露：earlyWatchErrors 清零=纵深防御——失败扫描收口即 released-unobserved-carry+slot-reaped，同槽再起状态设计上不可达（awaitingBind>0 的未绑定代永活不走初扫；carry 随失败代释放），行为隔离由槽回收承担，清零防未来回收策略变更**；变异踩坑两记：continue 打在内层 for 上=死循环假杀伤（改 break）；vitest -t 过滤下替身空读队列让重扫挂起=假绿（N4 补 4 条备用读后暴露真根因） | 七杀一披露 | ✅ |
