...
beforeEach(() => {
  narrow = false;
  listeners = new Set();
  vi.stubGlobal('matchMedia', vi.fn((query: string) => ({ matches: narrow, media: query,
...
function resize(mobile: boolean) { act(() => { narrow = mobile; listeners.forEach(callback => callback()); }); }
...
describe('双布局壳与导航', () => {
  ...
  it('加载与连接异常静态场景不声称成功或重试已受理', async () => {
    const user = userEvent.setup(); mount({ initialState: 'loading' });
  ...
  it('手机新建保留模型下拉；不创建真实会话，Esc 返回新建按钮', async () => {
    narrow = true; const user = userEvent.setup(); mount();
  ...
  it('不把 HTML 或未知 part 当活动内容或权限卡渲染', async () => {
    ...
    const { container } = mount({ adapter: { ...fixtureAdapter, sessions: [{ ...sample, messages: [{ id: 'unsafe', seq: 1, role: 'assistant', parts: [
